...
interface AEvent { id: number; title: string; category: string; startTime: string; isPublished: boolean; isFeatured: boolean; business: { name: string; slug: string } | null }
interface AOffer { id: number; title: string; type: string; badge: string; isActive: boolean; isFeatured: boolean; business: { name: string; slug: string } | null }
      ...
      <div className="mt-5 space-y-2">
        {tab === "events" && events.map((e) => (
          <div key={e.id} className="card flex flex-wrap items-center gap-3 p-4">
            ...
            <button onClick={async () => { await adminApi.patch(`/api/admin/events/${e.id}`, { isFeatured: !e.isFeatured }); load(); }} className={`chip ${e.isFeatured ? "chip-active" : ""}`}>{e.isFeatured ? "★ Featured" : "Feature"}</button>
        ...
        {tab === "offers" && offers.map((o) => (
          <div key={o.id} className="card flex flex-wrap items-center gap-3 p-4">
            <div className="min-w-0 flex-1">
              <p className="font-semibold text-ink">{o.title} <span className="chip !py-0 !text-[10px]">{(o.badge || o.type).replace(/_/g, " ")}</span></p>
              <p className="text-xs text-muted">{o.business ? <Link to={`/business/${o.business.slug}`} className="text-brand">{o.business.name}</Link> : "—"}</p>
            </div>
            <button onClick={async () => { await adminApi.patch(`/api/admin/offers/${o.id}`, { isFeatured: !o.isFeatured }); load(); }} className={`chip ${o.isFeatured ? "chip-active" : ""}`}>{o.isFeatured ? "★ Featured" : "Feature"}</button>
